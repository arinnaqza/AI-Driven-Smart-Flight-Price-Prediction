...
- R-squared ($$R^2$$) Score: measures how much of the variability in flight prices the model explains. Higher values indicate better performance.
  
We evaluate all models consistently using these metrics on the unseen test set, ensuring that the comparison is fair and reliable.

**B. Performance Analysis**

Our modeling approach follows a three-stage progression:
**1. Baseline Linear Regression**
Linear Regression provides the minimum performance reference.
This baseline MAE sets the standard that all advanced models must beat.
Because flight prices exhibit non-linear behavior, Linear Regression is expected to perform modestly, but it allows us to quantify how much improvement advanced models offer.
   
**2. Initial Gradient Boosting Models (LightGBM and XGBoost)**
 We next evaluated two tree-based Gradient Boosting Models:
   - LightGBM
   - XGBoost
Both models significantly improved upon the baseline, confirming that non-linear models are better suited for capturing the complex relationships in flight pricing.
LightGBM achieved a strong reduction in MAE compared to Linear Regression.
...
While LightGBM was initially tuned using RandomizedSearchCV, the tuned model still did not surpass XGBoost in accuracy.
Even without heavy tuning, XGBoost consistently produced the lowest MAE of all models tested.

**3. Model Comparison Results**
Below are the evaluation outputs for the initial LightGBM model and the final XGBoost model. These screenshots show the MAE and R² values exactly as generated by the model evaluation function.

LightGBM
<img width="402" height="72" alt="Screenshot 2025-12-11 at 10 40 42 am" src="https://github.com/user-attachments/assets/ae25231c-b2da-4089-9f83-798ee73d7170" />

XGBoost
<img width="410" height="80" alt="Screenshot 2025-12-11 at 10 40 51 am" src="https://github.com/user-attachments/assets/a968966b-66db-4e1b-8c13-6fa45c3b2370" />
      
**4. Final Model Selection**
   
After comparing all models on the unseen test set,
XGBoost achieved the lowest Mean Absolute Error (MAE) among all candidates, outperforming both the baseline Linear Regression and the LightGBM models, including the tuned version.
Because MAE directly represents prediction error in KRW,
the model with the lowest MAE is considered the most practical and reliable for real-world use.

Therefore:
Our final champion model is XGBoost.
It provides the most accurate and consistent flight price predictions and is the model integrated into our final analysis and application.
